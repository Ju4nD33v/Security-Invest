...
export async function GET(request: NextRequest, context: { params: Promise<{ symbol: string }> }) {
  try {
    ...
    await enforceRateLimit(request, "market:history", symbol, 20, 60);
    return apiJson({ data: await new FmpProvider().history(symbol), source: "FMP", retrievedAt: new Date().toISOString() }, request);
