...
export async function GET(request: NextRequest, context: { params: Promise<{ symbol: string }> }) {
  try { const { symbol } = symbolParamsSchema.parse(await context.params); await enforceRateLimit(request, "analysis:sentiment", symbol, 10, 60); return apiJson(await new AlphaVantageProvider().sentiment(symbol), request); }
